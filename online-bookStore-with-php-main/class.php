        ...
        function data_error($fname , $lname , $email , $mobile , $profile_pic , $n_card_pic , $pass){
            try{
                if(empty($fname) or empty($lname) or empty($email) or empty($mobile) or empty($pass))
                    throw new Exception();
//_______________ firstname
                try{
                    if(!preg_match('/^[А-Яа-я]*$/',$fname))
                        throw new Exception('pregmatch');
                    if(strlen($fname)>25)
                        throw new Exception('lengh');
                }
                catch(Exception $e){
                    $this->error=true;
                    if($e->getMessage()=='pregmatch')
...
//_____________ save data
        function insert_data(){
            ...
            if($resault->num_rows>0){
                ...
                VALUE('$this->fname' , '$this->lname' , '$this->email' , '$this->mobile' , '$this->pass' , '$activation_code' , 'no' , '$this->profile_pic' , '$this->n_card_pic' , 'no' , 'no subscription')";
                $resault=$this->mysql->query($sql);
                $this->mysql->close();
                if($resault===true){
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    class login{
        ...
        private $mysql;
//_________________________ login error check
...
//_________________________ connect to database
        function connect_db($db_host , $db_username , $db_password , $db_name){
            $this->mysql= new mysqli($db_host , $db_username , $db_password , $db_name);
            if($this->mysql->connect_error)
                die('connection error');
...
//_________________________ check input data to access login
...
//_________________________ active account  
...
//_______________Extractiing user info
...
//_______________save payments data
...
//_________________________ download history
...
//_________________________ payment history
...
//_________________________ edit user profile
